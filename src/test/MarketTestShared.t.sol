...
contract MarketTestShared is DSTest, Shared {
    ...
    function keepReservesAndBalsInCheck() internal {
        ...
        (uint reserve0, uint reserve1) = Market(marketAddress).getOutcomeReserves();

        (address tokenC, address token0, address token1) = Market(marketAddress).getTokenAddresses();
