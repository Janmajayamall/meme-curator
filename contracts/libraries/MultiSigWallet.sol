...
contract MultiSigWallet {
     ...
     *  Constants
     */
    uint immutable public MAX_OWNER_COUNT;
    ...
    constructor(address[] memory _owners, uint _required, uint maxCount) 
    {
        require(_required > 0);
        require(_required <= _owners.length && _owners.length <= maxCount);
        for (uint i=0; i<_owners.length; i++) {
            isOwner[_owners[i]] = true;
        }
        owners = _owners;
        required = _required;
        MAX_OWNER_COUNT = maxCount;
    }
